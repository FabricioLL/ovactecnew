...
FOR EACH ROW BEGIN
    ...
    where new.status = 'VALID';
end;
//
DELIMITER ;

DELIMITER //
CREATE TRIGGER `tr_updStockCompraRegresar` AFTER UPDATE ON `purchases`
FOR EACH ROW BEGIN
    UPDATE products p
        JOIN purchase_details di
            ON di.product_id = p.id
...
DELIMITER //
CREATE TRIGGER `tr_updStockVentaRegresar` AFTER UPDATE ON `sales`
FOR EACH ROW BEGIN
    UPDATE products p
        JOIN sale_details dv
            ON dv.product_id = p.id
